...
class CUIManager:
    ...
    def __init__(self, objects: list[CUIObject], onSurface: bool = False, pos: Sequence[float] = None,
                 scale: bool = False, preres: Sequence[float] = None, postres: Sequence[float] = None):
        ...
        if scale:
            if preres is None or postres is None:
                raise TypeError("Missing res value!")

            if self.offset is not None:
                raise TypeError("Does not work with offset!")

            self.scale = (preres[0] / postres[0], preres[1] / postres[1])

    def set_scale(self, preres: Sequence[float], postres: Sequence[float]):
        self.scale = (preres[0] / postres[0], preres[1] / postres[1])
...
class CScreen:
    ...
    VER 0.1.0: Removed `Scrap` as it is automatically started upon screen declaration
...
class CScaleScreen(CScreen):
    ...
    def __init__(self, size: Sequence[float] = (0, 0), flags: int = 0, display: int = 0,
        ...
        if prescaledSize == (0, 0):
            prescaledSize = self.size
        self.prescaledSurface = pygame.Surface((prescaledSize[0], prescaledSize[1]), flags)
    ...
    def draw(self, obj, pos: Sequence[float] = None) -> None:
        if isinstance(obj, (CUIObject, CRect, BaseObject, pygame.FRect, pygame.Rect)):
            ...
            self.prescaledSurface.blit(obj,
                                       pos)  # noqa:unexpected_types ; its saying "Sequence[float] is not Sequence[float]"
...
def init():
    ...
    """
    pygame.init()
    print("pygame_UI: initialized pygame-ce based UI system.")
    print(f"pygame_UI: VER: `{VER}` VER_STRING: `{VER_STRING}`")
